...
# Maps country code -> (SerpApi gl, DuckDuckGo kl)
REGION_MAP = {
    # Europe
    "uk": ("gb", "uk-en"),
    "ie": ("ie", "ie-en"),
...
def search_google(keyword, num=10, region=""):
    api_key = os.environ.get("SERPAPI_KEY", "")
    gl, kl = REGION_MAP.get(region, ("", ""))
    ...
    if api_key:
        ...
        try:
            params = {"q": keyword}
            if kl:
                params["kl"] = kl  # DuckDuckGo region param
            r = requests.get(
...
def api_search():
    ...
    urls, error = search_google(keyword, num, region)
